...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    h2_expo = np.exp(h2)
    expo_sum = np.sum(h2_expo,axis=1,keepdims=True)
    scores = h2_expo / expo_sum
    """
    ...
    grads["b2"] = scores.sum(axis=0)
    grads["W2"] = h1.T.dot(scores) / N
    relu_back = scores.dot(W2.T)
    relu_back[fc1<=0] = 0
    grads["W1"] = X.T.dot(relu_back)
    grads["b1"] = np.sum(relu_back,axis=0)
    grads["W1"] += 2 * reg * W1
    grads["W2"] += 2 * reg * W2
    """
